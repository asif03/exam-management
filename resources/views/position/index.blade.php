    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">Invisilator's Position List</h5>
                        <x-alert />
                                ...
                                <tbody>
                                    @foreach ($roles as $role)
                                        <tr>
                                            ...
                                            <td>
                                                <a class="btn btn-sm"
                                                    href="{{ route('exam-schedule-roles.edit', $role->id) }}">
                                                ...
                                                @if ($role->active == 1)
                                                    ...
                                                    <form class="hidden" id="{{ 'form-inactive-' . $role->id }}"
                                                        action="{{ route('exam-schedule-role.inactive', $role->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                    </form>
                                                @else
                                                    ...
                                                    <form class="hidden" id="{{ 'form-active-' . $role->id }}"
                                                        action="{{ route('exam-schedule-role.active', $role->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                    </form>
